  ...
  async fetch(
    ...
    if (
      ...
      try {
        ...
        // Checkout creates a new Stripe subscription. Until plan changes are
        // handled through Stripe Billing Portal, prevent a paid account from
        // accidentally creating a second concurrent subscription.
        if (
          validStripePlan(
            currentPlan.plan
          )
          ...
          return Response.json(
            {
              error:
                currentPlan.plan === requestedPlan
                  ? "This plan is already active."
                  : "Paid plan changes are not available in Sandbox yet.",
              code:
                currentPlan.plan === requestedPlan
                  ? "plan_already_active"
                  : "paid_plan_change_unavailable",
              currentPlan:
                currentPlan.plan,
              requestedPlan,
              entitlementChanged:
