...
const worker = fs.readFileSync(new URL("../src/index.js", root), "utf8")

for (const locale of ["en", "hu", "de", "fr", "es"]) assert(page.includes(`${locale}:{`), `locale copy missing: ${locale}`)
assert(worker.includes('url.pathname === "/api/x30/capability"'), "bounded capability endpoint required")
assert(worker.includes("authenticated: false, canaryEligible: false, generationEnabled: false"), "guest capability must fail closed")
assert(worker.includes("authenticated: true, canaryEligible, generationEnabled: canaryEligible && x30ProviderEnabled(env)"), "authenticated capability must expose only bounded booleans")
assert(page.includes('fetch("/api/x30/capability"') && page.includes("!capability?.generationEnabled"), "generation control must be gated by server capability")
assert(!worker.slice(worker.indexOf('url.pathname === "/api/x30/capability"'), worker.indexOf('url.pathname === "/api/x30/generate"')).includes("email:"), "capability response must not expose email")
...
assert(page.includes("if(isGenerating||briefState!==\"preview-only\"||!capability?.generationEnabled) return"), "duplicate, unapproved or unavailable submissions must be blocked")
...
assert(page.includes("disabled={isGenerating||briefState!==\"preview-only\"||!capability?.generationEnabled}"), "generate action must expose disabled server-capability semantics")
