...
await service.create(record,context)
...
assert(events.some(event=>event.action==="memory.created")&&events.some(event=>event.action==="memory.deleted")&&events.some(event=>event.action==="memory.persistence_denied"),"auditable operations")
console.log("Durable memory audit passed: user/workspace/project isolation, permissions, deletion, expiry, provenance, prohibited-data denial, approved persistence, Brain gateway and adapter behavior")
