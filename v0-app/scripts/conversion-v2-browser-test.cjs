...
;(async () => {
  ...
  try {
    ...
    await page.goto(base + '/challenge?utm_source=cold_email&utm_medium=email&utm_campaign=roofers_sep26&utm_content=followup_1')
    await page.waitForFunction(() => window.dataLayer?.some(x => x[0] === 'event' && x[1] === 'page_view'))
    const emailPageView = (await events()).filter(x => x[1] === 'page_view').at(-1)
    assert.equal(emailPageView[2].campaign_source, 'cold_email')
    assert.equal(emailPageView[2].campaign_medium, 'email')
    assert.equal(emailPageView[2].campaign_name, 'roofers_sep26')
    assert.equal(emailPageView[2].campaign_content, 'followup_1')
