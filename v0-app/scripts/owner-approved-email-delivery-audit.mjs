...
try {
  ...
  assert.equal((await handler.fetch(post({ actionId: action.id, confirmationDigest: "0".repeat(64) }), { DB: disabled.db, EMAIL_DELIVERY_ENABLED: "enabled" }, {})).status, 409)
  ...
  const sent = await handler.fetch(post({ actionId: action.id, confirmationDigest: digest }), { DB: success.db, EMAIL_DELIVERY_ENABLED: "enabled", RESEND_API_KEY: "mock" }, {})
  ...
  assert.equal((await handler.fetch(post({ actionId: action.id, confirmationDigest: digest }), { DB: success.db, EMAIL_DELIVERY_ENABLED: "enabled", RESEND_API_KEY: "mock" }, {})).status, 409, "duplicate send must be blocked")
  const failed = fakeDb(); globalThis.fetch = async () => new Response("no", { status: 503 })
  assert.equal((await handler.fetch(post({ actionId: action.id, confirmationDigest: digest }), { DB: failed.db, EMAIL_DELIVERY_ENABLED: "enabled", RESEND_API_KEY: "mock" }, {})).status, 503)
  const timeout = fakeDb(); globalThis.fetch = async () => { const error = new Error("timeout"); error.name = "AbortError"; throw error }
  const timeoutResponse = await handler.fetch(post({ actionId: action.id, confirmationDigest: digest }), { DB: timeout.db, EMAIL_DELIVERY_ENABLED: "enabled", RESEND_API_KEY: "mock" }, {})
