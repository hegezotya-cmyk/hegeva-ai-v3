...
assert(route.includes("getLoggedInUserFn") && source.indexOf("evaluateAIBotCanaryPreflight") < start && route.indexOf("admitAIBotCanary") < route.indexOf("invokeWorkersAiText"), "pre-authorization ordering changed")
...
assert(source.includes("ai_provider_usage") && source.includes("prepaidReserved") && source.includes("included-allowance-unavailable"), "persisted included allowance check missing")
...
const envBase = { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "enabled", AI_BOT_CANARY_EMAIL: "owner@example.test", AI_PROVIDER_MODEL: "@cf/meta/llama-3.1-8b-instruct-fast", AI_DOCUMENTED_DAILY_NEURON_ALLOCATION: "10000", AI_DAILY_NEURON_CEILING: "7000", AI_DAILY_REQUEST_CEILING: "1", AI_PER_USER_CEILING: "1", AI_PER_WORKSPACE_CEILING: "1", AI_CONCURRENCY_CEILING: "1", AI_MAX_INPUT_TOKENS: "200", AI_MAX_OUTPUT_TOKENS: "100", AI_TIMEOUT_MS: "10000", ASSETS: { fetch: () => new Response("ok") } }
const makeD1 = () => {
  const initialRevision = new Date().toISOString()
  const s = { data: JSON.stringify([{ id: "bot-test", schemaVersion: "0.1", name: "Canary", purpose: "readiness", instructions: "short", knowledgeScope: "workspace", permittedTools: ["none"], enabled: false, approvalState: "not-requested", executionState: "not-started", approvedAt: null, approvalExpiresAt: null, approvedByActorHash: null, approvalVersion: 1, approvalRevision: null, createdAt: initialRevision, updatedAt: initialRevision }]), updatedAt: initialRevision, auth: 0, admissions: 0, reservations: 0, events: 0, operations: 0, history: 0, usage: 0 }
  const prepare = (sql) => ({
    bind(...args) {
      return {
        async first() {
