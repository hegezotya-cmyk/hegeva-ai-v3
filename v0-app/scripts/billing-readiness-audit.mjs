...
assert(/Only \$\{expectedMode\} checkout is allowed in this build/.test(worker), "The backend must reject checkouts that do not match the configured payment mode")
assert(/getPaymentMode\(env\)/.test(worker) && /isStripeModeLive\(env\)/.test(worker), "The backend must derive the payment mode from the environment")
assert(/isStripeSecret\(env\)/.test(worker), "The backend must validate a configured Stripe secret")
...
assert(/if \(event\?\.livemode === true && !liveMode\)/.test(ledger), "The ledger must reject live Stripe events before any D1 billing writes when in test mode")
assert(/if \(event\?\.livemode === false && liveMode\)/.test(ledger), "The ledger must reject test Stripe events when in live mode")
...
assert(/Only Stripe billing is available in this build/.test(worker), "The customer portal must be gated on the Stripe provider")
...
console.log("Billing readiness audit passed: fail-closed mode gating, verified idempotent webhooks, lifecycle handling, and backend-authoritative entitlement")
