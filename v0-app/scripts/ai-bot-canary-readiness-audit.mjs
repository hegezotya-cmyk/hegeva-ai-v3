...
const env = { DB: db, AI: { async run() { providerCalls += 1; return { response: "unexpected" } } }, AI_PROVIDER_ENABLED: "disabled", AI_GLOBAL_KILL_SWITCH: "enabled", AI_OWNER_CANARY_ENABLED: "disabled", FINANCIAL_GUARD_ENABLED: "disabled", AI_BOT_CANARY_EMAIL: actor.email, AI_PROVIDER_MODEL: "@cf/meta/llama-3.1-8b-instruct-fast", AI_DOCUMENTED_DAILY_NEURON_ALLOCATION: "10000", AI_DAILY_NEURON_CEILING: "7000", AI_DAILY_REQUEST_CEILING: "1", AI_PER_USER_CEILING: "1", AI_PER_WORKSPACE_CEILING: "1", AI_CONCURRENCY_CEILING: "1", AI_MAX_INPUT_TOKENS: "200", AI_MAX_OUTPUT_TOKENS: "100", AI_TIMEOUT_MS: "10000" }
...
const nonOwnerHandler = createRequestHandler({ getLoggedInUserFn: async () => ({ id: "readiness-non-owner", email: "non-owner@example.test" }) })
const nonOwnerResponse = await nonOwnerHandler.fetch(new Request("https://hegevaai.co.uk/api/ai-bot/canary-readiness", { method: "POST", headers: { cookie: "session=redacted", "content-type": "application/json" }, body: JSON.stringify({ profileId: profile.id }) }), env, {})
const nonOwnerBody = await nonOwnerResponse.json()
assert(nonOwnerResponse.status === 403 && nonOwnerBody.reason === "owner-identity-mismatch", "non-owner readiness must be denied")
assert(providerCalls === 0 && mutations === 0, "non-owner readiness must not mutate or invoke provider")
console.log("Integrated non-owner proof passed: 403 owner identity mismatch, zero mutations, zero provider calls")
const cases = [["kill-switch-active", { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "enabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "enabled" }], ["canary-disabled", { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "disabled", FINANCIAL_GUARD_ENABLED: "enabled" }], ["financial-guard-disabled", { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "disabled" }], ["model-invalid", { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "enabled", AI_PROVIDER_MODEL: "wrong-model" }], ["allocation-invalid", { AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "enabled", AI_DOCUMENTED_DAILY_NEURON_ALLOCATION: "bad" }]]
...
const readyEnv = { ...env, AI_PROVIDER_ENABLED: "enabled", AI_GLOBAL_KILL_SWITCH: "disabled", AI_OWNER_CANARY_ENABLED: "enabled", FINANCIAL_GUARD_ENABLED: "enabled" }
