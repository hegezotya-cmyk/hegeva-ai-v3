...
for(const token of ["getLoggedInUser","loadStoredAIBotProfile","AI_BOT_CANARY_EMAIL","sha256Hex","approvalState: \"owner-approved\"","approvalExpiresAt","approvalVersion","approvalRevision: approvedAt","updatedAt=?2","updatedAt=?4","status: 409"])assert(route.includes(token),`approval contract missing ${token}`)
