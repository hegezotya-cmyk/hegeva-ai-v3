...
assert((panel.match(/fetch\(/g) || []).length === 2, "panel fetch count is not bounded")
assert(panel.includes('fetch("/api/workspace/ai-bot-profiles"') && panel.includes('fetch("/api/ai-bot/renew-approval"'), "panel fetch endpoints are not the approved bounded paths")
