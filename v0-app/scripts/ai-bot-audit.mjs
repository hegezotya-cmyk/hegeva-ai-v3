...
for (const match of source.matchAll(/fetch\s*\(\s*["']([^"']+)["']/g)) if (!["/api/ai-bot/approve", "/api/ai-bot/renewal-readiness", "/api/ai-bot/canary-readiness", "/api/ai-bot/canary-once"].includes(match[1])) throw new Error(`AI Bot client fetch is not an approved authenticated boundary: ${match[1]}`)
