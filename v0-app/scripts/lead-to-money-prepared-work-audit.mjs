...
const quoteBrief=leadResult.preparedActions.find(x=>x.kind==="quote-brief"&&x.sourceIds.includes("lead1"))
assert(quoteBrief,"qualified lead gap must prepare an owner-review quote brief")
assert.equal(quoteBrief.status,"prepared")
assert.equal(quoteBrief.reason,"quote-brief-owner-review-pending")
assert.match(quoteBrief.content,/before creating any quote document/i)
assert(!leadResult.preparedActions.some(x=>x.kind==="quote-create"))
