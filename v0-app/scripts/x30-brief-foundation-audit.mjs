...
assert(page.includes("const tokenLabels") && page.includes("t.domains[item]") && page.includes("t.visual[item]") && page.includes("t.capabilities[capability]") && page.includes("t.sensitivity[item]"), "all internal enum tokens must use localized presentation labels")
assert(page.includes('disabled={briefState!=="ready-for-review"}') && page.includes('aria-disabled={briefState!=="ready-for-review"}') && page.includes('disabled={isGenerating||briefState!=="preview-only"||!capability?.generationEnabled}'), "approval and generation gates must follow the canonical review states and server capability")
assert(page.includes('briefState==="ready-for-review"&&<p'), "awaiting-owner-approval copy must be visible during review")
