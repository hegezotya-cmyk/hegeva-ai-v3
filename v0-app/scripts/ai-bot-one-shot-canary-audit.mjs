...
for (const token of ["getLoggedInUser", "AI_BOT_CANARY_EMAIL", "loadCanaryProfile", "parseProviderFlags", "getWorkersAiCanaryConfig", "crypto.randomUUID()", "sha256Hex(rawToken)", "5 * 60 * 1000", "admitAIBotCanary", "invokeWorkersAiText", "finishAIBotOperation", "revokeUnusedCanaryAuthorization"]) assert(route.includes(token), `canary contract missing ${token}`)
...
assert(route.indexOf("getLoggedInUser") < route.indexOf("loadCanaryProfile") && route.indexOf("admitAIBotCanary") < route.indexOf("invokeWorkersAiText"), "provider ordering is unsafe")
