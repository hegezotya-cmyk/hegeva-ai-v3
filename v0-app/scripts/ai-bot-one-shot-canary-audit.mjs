...
for (const token of ["getLoggedInUser", "AI_BOT_CANARY_EMAIL", "loadCanonicalAIBotProfile", "parseProviderFlags", "getWorkersAiConfig", "crypto.randomUUID()", "sha256Hex(rawToken)", "5 * 60 * 1000", "admitAIBotCanary", "invokeWorkersAiText", "finishAIBotOperation", "revokeUnusedCanaryAuthorization"]) assert(route.includes(token), `canary contract missing ${token}`)
...
assert(route.indexOf("getLoggedInUser") < route.indexOf("loadCanonicalAIBotProfile") && route.indexOf("admitAIBotCanary") < route.indexOf("invokeWorkersAiText"), "provider ordering is unsafe")
assert(/keys\.length !== 1/.test(route) && /keys\[0\] !== "profileId"/.test(route), "client payload is not restricted to profileId")
assert(/globalDailyCeiling:\s*1/.test(route) && /userDailyCeiling:\s*1/.test(route) && /workspaceDailyCeiling:\s*1/.test(route), "one-request ceilings missing")
assert(/max\(1, Math\.min\(providerConfig\.dailyNeuronCeiling, 1\)/.test(route), "neuron bound missing")
assert(route.includes("providerConfig.concurrencyCeiling !== 1"), "concurrency bound missing")
