...
const coreModule=await import(`data:text/javascript;base64,${Buffer.from(coreSource).toString("base64")}`)
const coreResult=coreModule.projectLeadToMoney({customers:[{id:"lead",customerStatus:"lead"},{id:"active",customerStatus:"active"}],invoices:[{id:"q1",type:"quote",status:"sent",customerId:"lead"},{id:"i1",type:"invoice",status:"paid",customerId:"active"}],messages:[]})
assert.ok(coreResult.some(x=>x.stage==="follow-up"&&x.status==="needs-attention"&&x.sourceIds.includes("q1")))
assert.ok(coreResult.some(x=>x.stage==="payment"&&x.status==="complete"&&x.sourceIds.includes("i1")))
assert.ok(coreResult.some(x=>x.stage==="repeat-business"&&x.sourceIds.includes("active")))
assert.ok(coreResult.every(x=>x.sourceIds.length>0))
console.log("Lead-to-Money Core projection audit passed.")
