...
import { ArrowRight, Braces, Bot, CheckCircle2, Hammer, Megaphone, Rocket, Sparkles, Video, Wrench } from "lucide-react"
...
const aiBotsCopy = {
  en: ["AI Bots", "Controlled bot profiles with owner approval and safe execution gates."],
  hu: ["AI Botok", "Korlátozott botprofilok tulajdonosi jóváhagyással és biztonságos végrehajtási kapukkal."],
  de: ["AI-Bots", "Kontrollierte Botprofile mit Besitzerfreigabe und sicheren Ausführungsgrenzen."],
  fr: ["Bots IA", "Profils de bots contrôlés avec approbation du propriétaire et garde-fous d’exécution."],
  es: ["Bots de IA", "Perfiles de bots controlados con aprobación del propietario y límites de ejecución seguros."],
} as const
...
function StudioWorkflow(){
 const {t,locale}=useI18n();const c=getStudioCopy(locale);const aiBots=aiBotsCopy[locale]
 const stages=[
 ...
 return <div className="studio-workflow mt-9"><div className="studio-process"><header><p>HEGEVA engineering flow</p><span>REQUEST → SPEC → BUILD → VERIFY → RESULT</span></header>{stages.map(({number,label,icon:Icon,title,desc,href})=><Link href={href} key={number}><span>{number}</span><div><small>{label}</small><h2>{title}</h2><p>{desc}</p></div><Icon aria-hidden/><ArrowRight className="studio-arrow" aria-hidden/></Link>)}<Link href="/app-studio/advertising"><span>04</span><div><small>CAMPAIGN</small><h2>Advertising Studio</h2><p>Prepare, save and review bounded campaign briefs.</p></div><Megaphone aria-hidden/><ArrowRight className="studio-arrow" aria-hidden/></Link><Link href="/app-studio/video-ad-studio"><span>05</span><div><small>STORYBOARD</small><h2>Video Ad Studio</h2><p>Build a production-ready video specification.</p></div><Video aria-hidden/><ArrowRight className="studio-arrow" aria-hidden/></Link><Link href="/app-studio/ai-bots" className="ai-bots-studio-entry"><span>06</span><div><small>CONTROLLED BETA</small><h2>{aiBots[0]}</h2><p>{aiBots[1]}</p></div><Bot aria-hidden/><ArrowRight className="studio-arrow" aria-hidden/></Link></div><aside><X20Card/><Link href="/app-studio/x30-alpha" className="x30-alpha-entry"><div><Braces aria-hidden/><span>INTERNAL ALPHA</span></div><h2>X30 structured rendering</h2><p>Inspect the safe schema, domain direction and deterministic renderer that demonstrate HEGEVA’s next generation.</p><span>{c.open}<ArrowRight aria-hidden/></span></Link></aside></div>
}
