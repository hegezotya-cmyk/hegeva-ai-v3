    ...
    <div className="mt-10 grid gap-5 lg:grid-cols-3">
      {plans.map((plan) => <section key={plan.key} className={`glass-panel flex rounded-3xl p-6 sm:p-8 ${"featured" in plan && plan.featured ? "border-primary/50 ring-1 ring-primary/25" : ""}`}>
        <div className="flex w-full flex-col">
          ...
          {plan.key === "basic" ? <button type="button" disabled className="mt-8 rounded-xl border border-border px-5 py-3 text-sm font-semibold text-muted-foreground">{c.current}</button> : <button type="button" disabled={opening !== null || isPending} onClick={() => void checkout(plan.key)} className="mt-8 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-opacity disabled:opacity-60">{opening === plan.key ? c.opening : !session?.user && !isPending ? c.signIn : c.choose}</button>}
        </div>
      </section>)}
    </div>
