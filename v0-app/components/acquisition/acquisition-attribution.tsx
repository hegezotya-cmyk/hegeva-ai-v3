...
function record(event:SafeEvent,path:string){try{const existing=JSON.parse(localStorage.getItem(key)||"[]"),events=Array.isArray(existing)?existing:[];events.push({event,path,at:new Date().toISOString()});localStorage.setItem(key,JSON.stringify(events.slice(-20)));window.dispatchEvent(new CustomEvent("hegeva:analytics-event",{detail:{event,path}}))}catch{}}
