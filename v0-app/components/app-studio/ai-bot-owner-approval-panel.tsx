...
export function AIBotOwnerApprovalPanel() {
  ...
  function update(profileId: string, expiresAt: string, version: number) { setItems(current => current.map(item => item.id === profileId ? { ...item, approvalState: "owner-approved", approvalExpiresAt: expiresAt, approvedAt: new Date().toISOString(), approvedByActorHash: null, approvalVersion: version } : item)); setMessage(copy.enabled) }
