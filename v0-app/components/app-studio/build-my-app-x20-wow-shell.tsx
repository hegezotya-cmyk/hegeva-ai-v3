...
import { BuildMyAppX20Wow } from "@/components/app-studio/build-my-app-x20-wow"

export function BuildMyAppX20WowShell() {
  return (
    <div className="relative overflow-hidden">
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[900px] bg-[radial-gradient(circle_at_18%_8%,rgba(16,213,141,.18),transparent_34%),radial-gradient(circle_at_82%_12%,rgba(212,175,55,.11),transparent_28%)]" />
      <div className="mx-auto max-w-[1500px] px-4 pt-7 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-3xl border border-primary/20 bg-background/65 shadow-[0_30px_100px_rgba(0,0,0,.18)] backdrop-blur-xl">
          ...
          <div className="grid gap-px border-b border-border/70 bg-border/70 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              [MonitorSmartphone,"Responsive preview","Desktop · tablet · phone"],
              [Braces,"Verified HTML","Preview · Code · Split"],
              [Layers3,"Same-project tuning","Snapshots + safe restore"],
            ].map(([Icon,title,text]) => {
