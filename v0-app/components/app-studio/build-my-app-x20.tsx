...
const accessiblePatch = `<style data-hegeva-x20="accessible">:focus-visible{outline:3px solid #53e7b3!important;outline-offset:3px!important}@media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;animation:none!important;transition:none!important}}</style><script data-hegeva-x20="accessible">(()=>{document.querySelectorAll('input,textarea,select').forEach((el,i)=>{if(!el.getAttribute('aria-label')&&!el.getAttribute('aria-labelledby'))el.setAttribute('aria-label',el.getAttribute('placeholder')||el.getAttribute('name')||('Field '+(i+1)))});document.querySelectorAll('button').forEach((b,i)=>{if(!b.textContent?.trim()&&!b.getAttribute('aria-label'))b.setAttribute('aria-label','Action '+(i+1))})})();</script>`

const dashboardPatch = `<section data-hegeva-x20="dashboard" style="margin:18px auto;max-width:1180px;padding:18px;border:1px solid rgba(16,213,141,.25);border-radius:18px;background:#0d1c16;color:#eef7f3;font-family:Inter,system-ui,sans-serif"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap"><div><small style="color:#72dcb2;font-weight:800;letter-spacing:.12em">X20 DASHBOARD</small><h2 style="margin:.35rem 0">Live app overview</h2></div><span style="padding:8px 11px;border-radius:999px;background:rgba(16,213,141,.12);color:#7de6bc">Local app data</span></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-top:14px"><div style="padding:14px;border-radius:14px;background:#10251d"><small>Saved keys</small><strong id="x20-key-count" style="display:block;font-size:1.8rem">0</strong></div><div style="padding:14px;border-radius:14px;background:#10251d"><small>Interactive controls</small><strong id="x20-control-count" style="display:block;font-size:1.8rem">0</strong></div><div style="padding:14px;border-radius:14px;background:#10251d"><small>Sections</small><strong id="x20-section-count" style="display:block;font-size:1.8rem">0</strong></div></div></section><script data-hegeva-x20="dashboard">(()=>{const q=s=>document.querySelector(s);const safe=()=>{try{return localStorage.length}catch{return 0}};const k=q('#x20-key-count'),c=q('#x20-control-count'),s=q('#x20-section-count');if(k)k.textContent=String(safe());if(c)c.textContent=String(document.querySelectorAll('button,input,select,textarea,a[href]').length);if(s)s.textContent=String(document.querySelectorAll('main section,section,article').length)})();</script>`
...
function qualityScore(html: string) {
  ...
  const hasVerifiedX20Runtime = /data-hegeva-x20=["']safe-interactions["']/i.test(html)
  const genericOnly = /HEGEVA App Studio/i.test(html) && /Home[\s\S]{0,100}About[\s\S]{0,100}Contact/i.test(html)
  return (controls >= 6 ? 2 : controls >= 2 ? 1 : 0) + (sections >= 4 ? 2 : sections >= 2 ? 1 : 0) + (hasScript ? 1 : 0) + (hasStorage ? 1 : 0) + (hasVerifiedX20Runtime ? 2 : 0) - (genericOnly ? 3 : 0)
}
...
function buildInstruction(value: string, locale: string, mode: BuildMode, retry = false) {
  return ["You are HEGEVA Build My App X20, a premium product builder.", `Visible UI language: ${locale}.`, buildModeInstruction(mode), retry ? "The previous attempt was too generic. Regenerate a substantially more complete product with more real interactions." : "Create the first strong working build.", "Return ONLY one complete self-contained index.html with inline CSS and vanilla JavaScript. No dependencies and no external images.", "Build the actual product described by the idea, NOT a landing page, demo shell, Home/About/Contact template, placeholder panel or tutorial example.", "Every visible primary action must work locally. Forms must validate, add/edit/delete actions must update the UI, and useful app data must persist with localStorage and restore on reload.", "Use real product navigation, meaningful empty states, responsive layout, accessible labels and mobile-friendly controls.", "Include a useful overview/dashboard derived only from real local app state. Never invent revenue, customers, payment success or external-service results.", "Do not fake authentication, payments, email, cloud databases, deployment or external APIs. Clearly mark unavailable integrations when needed.", "Finish with valid closing body and html tags.", `APP IDEA: ${value}`].join("\n\n")
}
...
function healthChecks(html: string) {
  ...
  const storage = /localStorage|window\.name/i.test(html)
  ...
  return [
    { label: "Buttons", ok: buttons >= 1 && scripts },
    { label: "Forms", ok: forms >= 1 && fields >= 1 && scripts },
...
export function BuildMyAppX20() {
  ...
  useEffect(() => {
    try {
      ...
      if (storedVersion === STORAGE_VERSION && storedHtml && looksLikeHtmlDocument(storedHtml)) {
        setHtml(storedHtml)
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="rounded-3xl border border-primary/30 bg-primary/5 p-6 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{c.eyebrow}</p><h1 className="mt-3 text-4xl font-bold tracking-tight text-foreground sm:text-5xl">{c.title}</h1><p className="mt-4 text-sm leading-7 text-muted-foreground sm:text-base">{c.sub}</p></div><span className="rounded-full border border-gold/30 bg-gold/10 px-3 py-1 text-xs font-semibold text-gold">PRO · BETA</span></div></div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{progress.map((item,index)=><div key={item.label} className="glass-panel rounded-2xl p-4"><div className="flex items-center gap-3"><span className={`flex size-8 items-center justify-center rounded-full text-xs font-bold ${item.done ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"}`}>{index+1}</span><span className={item.done ? "text-sm font-semibold text-foreground" : "text-sm text-muted-foreground"}>{item.label}</span></div></div>)}</div>

      <section className="mt-6 glass-panel rounded-2xl p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-base font-semibold text-foreground">{c.choose}</h2><p className="mt-1 text-sm text-muted-foreground">{c.chooseSub}</p></div><span className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{buildMode.toUpperCase()}</span></div><div className="mt-4 grid gap-3 md:grid-cols-3">{buildModes.map(({key,title,text,icon:Icon})=>{const selected=buildMode===key;return <button key={key} type="button" onClick={()=>setBuildMode(key)} className={`rounded-2xl border p-4 text-left transition ${selected?"border-primary/60 bg-primary/10 shadow-[0_0_0_1px_rgba(16,213,141,.15)]":"border-border bg-background/35 hover:border-primary/30"}`}><div className="flex items-center justify-between gap-3"><Icon className={`size-5 ${selected?"text-primary":"text-muted-foreground"}`} /><span className={`text-xs font-bold ${selected?"text-primary":"text-muted-foreground"}`}>{selected?"SELECTED":""}</span></div><h3 className="mt-3 text-base font-semibold text-foreground">{title}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{text}</p></button>})}</div></section>

      <section className="mt-6 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]"><div className="glass-panel rounded-2xl p-5"><label htmlFor="x20-idea" className="text-sm font-semibold text-foreground">{c.idea}</label><textarea id="x20-idea" value={idea} onChange={(e)=>setIdea(e.target.value)} rows={8} placeholder={c.placeholder} className="mt-3 w-full rounded-xl border border-input bg-input/30 p-3 text-sm outline-none focus:border-primary/50" /><button type="button" onClick={()=>void build()} disabled={!idea.trim()||busy} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"><WandSparkles className="size-4" aria-hidden />{busy?c.building:c.build}</button><p className="mt-3 text-xs leading-relaxed text-muted-foreground">{c.saved}</p>{error&&<p role="alert" className="mt-4 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">{error}</p>}</div><div className="glass-panel rounded-2xl p-5"><h2 className="text-sm font-semibold text-foreground">{c.improve}</h2><div className="mt-3 grid gap-3 sm:grid-cols-2">{actions.map(({key,label,icon:Icon})=><button key={key} type="button" onClick={()=>improve(key)} disabled={!html||busy} className="flex items-center gap-3 rounded-xl border border-border bg-background/40 px-4 py-3 text-left text-sm font-medium transition-colors hover:border-primary/40 disabled:cursor-not-allowed disabled:opacity-40"><Icon className="size-4 text-primary" aria-hidden />{label}</button>)}</div>{!html&&<p className="mt-4 text-sm text-muted-foreground">{c.empty}</p>}</div></section>

      {html&&<section className="mt-6 glass-panel rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-base font-semibold text-foreground">{c.health}</h2><p className="mt-1 text-sm text-muted-foreground">{c.healthSub}</p></div><div className="text-right"><div className="text-xs uppercase tracking-wider text-muted-foreground">{c.score}</div><div className="text-3xl font-bold text-primary">{readiness}%</div></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">{checks.map(item=><div key={item.label} className={`rounded-xl border p-3 ${item.ok?"border-primary/25 bg-primary/5":"border-border bg-background/35"}`}><div className="flex items-center gap-2"><CheckCircle2 className={`size-4 ${item.ok?"text-primary":"text-muted-foreground"}`} /><span className="text-sm font-medium text-foreground">{item.label}</span></div></div>)}</div></section>}

      {html&&<section className="mt-6 grid gap-5 xl:grid-cols-2"><div className="glass-panel rounded-2xl p-5"><h2 className="text-sm font-semibold text-foreground">{c.preview}</h2><iframe title={c.preview} srcDoc={html} sandbox="allow-scripts" className="mt-3 h-[620px] w-full rounded-xl border border-border bg-white" /></div><div className="glass-panel rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-sm font-semibold text-foreground">{c.code}</h2><button type="button" onClick={()=>downloadTextFile("index.html",html,"text/html;charset=utf-8")} className="inline-flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm font-medium"><Download className="size-4" aria-hidden />{c.download}</button></div><pre className="mt-3 h-[620px] overflow-auto whitespace-pre-wrap rounded-xl border border-border bg-background/60 p-4 text-xs leading-relaxed">{html}</pre></div></section>}
    </div>
  )
}
