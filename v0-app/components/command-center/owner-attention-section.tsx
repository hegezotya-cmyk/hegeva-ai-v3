...
export function OwnerAttentionSection({locale,items}:{locale:Locale;items:AttentionItem[]}){
 const c=copy[locale]
 return <div className="border-t border-border px-4 py-6 sm:px-6"><p className="ve-eyebrow">{c.eyebrow}</p><h3 className="mt-1 font-display text-2xl font-semibold">{c.title}</h3><p className="mt-2 max-w-3xl text-sm text-muted-foreground">{c.sub}</p>{items.length?<div className="mt-4 grid gap-3 sm:grid-cols-2">{items.map(item=><Link key={item.id} href={item.targetHref} className="v4-priority-1 rounded-2xl border p-4 transition-colors hover:border-primary/35"><div className="flex items-center justify-between gap-3"><strong className="text-sm">{item.stage.replace(/-/g," ")}</strong><span className="v4-status-review rounded-full border border-gold/35 px-2 py-1 text-[.6rem] font-bold uppercase tracking-[.1em]">{c.prepared}</span></div><p className="mt-2 text-xs text-muted-foreground">{item.sourceIds.length} evidence record{item.sourceIds.length===1?"":"s"} · {item.nextStage||item.stage}</p><span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary">{c.review}<ArrowUpRight className="size-3.5"/></span></Link>)}</div>:<div className="v4-surface mt-4 rounded-2xl border p-4 text-sm text-muted-foreground"><Check className="mr-2 inline size-4 text-primary"/>{c.empty}</div>}</div>
}
