...
export function validateAIBotProfile(value: unknown): { ok: true; value: AIBotProfile } | { ok: false; code: string } {
  ...
  const record = value as Record<string, unknown>; const keys = Object.keys(record); const required = ["id", "schemaVersion", "name", "purpose", "instructions", "knowledgeScope", "permittedTools", "enabled", "approvalState", "executionState", "approvedAt", "approvalExpiresAt", "approvedByActorHash", "approvalVersion", "createdAt", "updatedAt"]
