...
function x20FragmentInstruction(message: string, language: StudioLocale) {
  return ["HEGEVA Build My App X20 compact application fragment generator.",`Visible UI language: ${language}.`,"Return ONLY compact semantic HTML markup for inside BODY.","IMPORTANT: start immediately with <main class=\"app\"> and include at least one <section>, one <form>, inputs and buttons.","Do not output doctype, html, head, body, style, script, Markdown or explanation.","Do not return plain text. Every visible sentence must be inside HTML elements.","Build an application interface, not a landing page. Keep it under about 1400 characters so it finishes completely.","Use simple data-entry forms and dashboard cards. HEGEVA will attach safe local interactions automatically.",`APP REQUEST:\n${message.slice(0, 1050)}`].join("\n\n")
}
...
function meaningfulFragment(fragment: string) {
  return fragment.length >= 180 && /<(main|section|article|form)(?:\s|>)/i.test(fragment) && /<(button|input|textarea|select)(?:\s|>)/i.test(fragment)
}

function fallbackX20Fragment(language: StudioLocale) {
  const t = language === "hu" ? {title:"Üzleti irányítópult",sub:"Kezeld a legfontosabb üzleti adatokat egy helyen.",dash:"Áttekintés",customers:"Ügyfelek",docs:"Dokumentumok",tasks:"Feladatok",name:"Név",add:"Hozzáadás",save:"Mentés",empty:"Még nincs mentett adat."} : {title:"Business dashboard",sub:"Manage your essential business data in one place.",dash:"Overview",customers:"Customers",docs:"Documents",tasks:"Tasks",name:"Name",add:"Add",save:"Save",empty:"No saved data yet."}
...
const X20_SAFE_SCRIPT = `<script data-hegeva-x20="safe-interactions">(()=>{const k='hegeva-x20-items',f=document.getElementById('hx-form'),i=document.getElementById('hx-name'),l=document.getElementById('hx-list'),c=document.getElementById('hx-count');if(!f||!i||!l)return;const read=()=>{try{return JSON.parse(localStorage.getItem(k)||'[]')}catch{return[]}},draw=()=>{const a=read();if(c)c.textContent=String(a.length);l.innerHTML=a.length?a.map((x,n)=>'<p>'+String(x).replace(/[&<>]/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[s]))+' <button type="button" data-del="'+n+'">×</button></p>').join(''):'No saved data yet.'};f.addEventListener('submit',e=>{e.preventDefault();const v=i.value.trim();if(!v)return;const a=read();a.push(v);localStorage.setItem(k,JSON.stringify(a));i.value='';draw()});l.addEventListener('click',e=>{const b=e.target.closest('[data-del]');if(!b)return;const a=read();a.splice(Number(b.dataset.del),1);localStorage.setItem(k,JSON.stringify(a));draw()});draw()})()</script>`
