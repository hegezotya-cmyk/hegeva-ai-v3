...
export interface DurableMemoryAdapter {
  ...
  purgeExpired(now: string, workspaceId: string): Promise<number>
...
export function classifyMemoryCandidate(candidate: MemoryCandidate, approved: boolean): { classification: MemoryWriteClass; reason: string } {
  ...
  if (candidate.projectId || candidate.requestedType === "project-artifact") return approved ? { classification: "PROJECT_MEMORY", reason: "The owner approved project-scoped memory." } : { classification: "SESSION_ONLY", reason: "Project memory requires explicit approval." }
  if (candidate.requestedType === "workspace") return approved ? { classification: "WORKSPACE_MEMORY", reason: "The owner approved workspace-scoped memory." } : { classification: "SESSION_ONLY", reason: "Workspace memory requires explicit approval." }
...
export class InMemoryDurableMemoryAdapter implements DurableMemoryAdapter {
  ...
  async purgeExpired(now: string, workspaceId: string) { let count = 0; for (const [id, item] of this.#records) if (item.workspaceId === workspaceId && item.expiresAt && Date.parse(item.expiresAt) <= Date.parse(now)) { this.#records.delete(id); count++ } return count }
...
export class MemoryService {
  ...
  async create(record: DurableMemoryRecord, context: MemoryAccessContext, approval: ApprovalState = "not-required") {
    ...
    if (decision.classification === "DO_NOT_STORE" || !isEligible(record, decision.classification, context)) {
      await this.emit("memory.persistence_denied", record, context)
      throw new Error("memory-persistence-denied")
    }
    const created = await this.adapter.create(record)
    await this.emit(record.type === "session" ? "memory.created" : "memory.persistence_approved", record, context)
    return created
  }

  async read(id: string, context: MemoryAccessContext) {
    const record = await this.adapter.read(id)
    if (!record) return null
    assertAccess(record, context, "memory.read")
    return isExpired(record, context.now) ? null : record
  }

  async query(query: MemoryQuery, context: MemoryAccessContext) {
    if (query.workspaceId !== context.workspaceId) throw new Error("memory-workspace-denied")
    const records = await this.adapter.query(query)
    return records.filter((record) => canAccess(record, context, "memory.read") && !isExpired(record, context.now))
  }

  async update(record: DurableMemoryRecord, context: MemoryAccessContext, approval: ApprovalState = "not-required") {
    assertAccess(record, context, "memory.write")
    validateRecord(record)
    const existing = await this.read(record.memoryId, context)
    if (!existing) throw new Error("memory-not-found")
    if (record.ownerUserId !== existing.ownerUserId || record.workspaceId !== existing.workspaceId || record.projectId !== existing.projectId || record.createdAt !== existing.createdAt) throw new Error("memory-scope-immutable")
    const decision = classifyMemoryCandidate({ ...record, requestedType: record.type }, approval === "approved")
    if (decision.classification === "DO_NOT_STORE" || !isEligible(record, decision.classification, context)) {
      await this.emit("memory.persistence_denied", record, context)
      throw new Error("memory-persistence-denied")
    }
    const updated = { ...record, version: existing.version + 1, updatedAt: context.now }
    await this.adapter.update(updated)
    await this.emit("memory.updated", updated, context)
    return updated
  }

  async delete(id: string, context: MemoryAccessContext) {
    const record = await this.read(id, context)
    if (!record) return false
    if (!context.permissions.includes("memory.delete")) throw new Error("memory-permission-denied")
    const deleted = await this.adapter.delete(id)
    if (deleted) await this.emit("memory.deleted", record, context)
    return deleted
  }

  async purgeExpired(context: MemoryAccessContext) {
    if (!context.permissions.includes("memory.manage")) throw new Error("memory-permission-denied")
    return this.adapter.purgeExpired(context.now, context.workspaceId)
  }

  private async emit(action: string, record: DurableMemoryRecord, context: MemoryAccessContext) {
    await this.audit?.record({ correlationId: record.correlationId, actor: { id: context.userId, kind: "user" }, action, resource: { id: record.memoryId, kind: "memory", version: record.version }, outcome: action.endsWith("denied") ? "denied" : "completed", summary: `${record.type} memory operation` })
  }
}
...
export class BrainMemoryGateway {
  ...
  persistApproved(record: DurableMemoryRecord, context: MemoryAccessContext, approval: ApprovalState) {
    if (approval !== "approved") throw new Error("memory-persistence-denied")
    return this.memory.create({ ...record, provenance: { ...record.provenance, source: "brain-proposal" } }, context, approval)
  }
}

function isEligible(record: DurableMemoryRecord, classification: MemoryWriteClass, context: MemoryAccessContext) {
  if (record.type === "session") return classification === "SESSION_ONLY" && record.retention === "session"
  if (record.type === "project-artifact") return classification === "PROJECT_MEMORY" && Boolean(record.projectId)
  if (record.type === "workspace") return classification === "WORKSPACE_MEMORY"
  return classification === "PERSIST_IF_APPROVED" && context.persistentMemoryEnabled && record.retention === "until-deleted"
}

function validateRecord(record: DurableMemoryRecord) {
  if (!record.memoryId.trim() || !record.ownerUserId.trim() || !record.workspaceId.trim() || !record.correlationId.trim()) throw new Error("memory-identity-required")
  if (!record.provenance.summary.trim()) throw new Error("memory-provenance-required")
  if (record.schemaVersion !== MEMORY_MODEL_VERSION || record.status !== "active" || !Number.isInteger(record.version) || record.version < 1) throw new Error("memory-record-invalid")
  for (const value of [record.createdAt, record.updatedAt, record.expiresAt].filter(Boolean) as string[]) if (!Number.isFinite(Date.parse(value))) throw new Error("memory-timestamp-invalid")
}
